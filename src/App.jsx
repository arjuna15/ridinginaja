...
function App() {
  ...
  const getShareStyles = () => {
    ...
    if (shareTheme === 'CLASSIC') {
      return {
        wrapper: { position: 'absolute', bottom: 0, left: 0, right: 0, padding: '24px 16px', background: 'linear-gradient(to top, rgba(0,0,0,1) 0%, rgba(0,0,0,0.8) 50%, transparent 100%)', zIndex: 50 },
        title: { fontSize: '18px', fontWeight: 'bold', color: '#fff', marginBottom: '2px' },
        date: { fontSize: '11px', color: '#aaa', marginBottom: '16px' },
        statRow: { display: 'flex', justifyContent: 'space-between', borderTop: '1px solid rgba(255,255,255,0.2)', paddingTop: '12px' },
        statVal: { fontSize: '18px', fontWeight: 'bold', color: '#fff' },
        statLbl: { fontSize: '10px', color: '#888', textTransform: 'uppercase' }
      };
    }
    // 2. NEON — Grid border box, scanline feel
    if (shareTheme === 'NEON') {
      return {
        wrapper: { position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, background: '#000', padding: '24px 16px', zIndex: 50, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' },
        title: { fontSize: '20px', fontWeight: '900', color: '#0f0', textShadow: '0 0 10px #0f0', marginBottom: '4px', fontFamily: 'monospace' },
        date: { fontSize: '11px', color: '#0a0', marginBottom: '20px', fontFamily: 'monospace' },
        statRow: { display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '12px', background: 'rgba(0,255,0,0.05)', padding: '14px', borderRadius: '4px', border: '1px solid #0f0', boxShadow: '0 0 20px rgba(0,255,0,0.1), inset 0 0 20px rgba(0,255,0,0.05)' },
        statVal: { fontSize: '16px', fontWeight: 'bold', color: '#0f0', textShadow: '0 0 8px #0f0', fontFamily: 'monospace' },
        statLbl: { fontSize: '9px', color: '#0a0', textTransform: 'uppercase', fontFamily: 'monospace', letterSpacing: '1px' }
      };
    }
    // 3. MINIMAL — Floating white card, top position
    if (shareTheme === 'MINIMAL') {
      return {
        wrapper: { position: 'absolute', top: '24px', left: '16px', right: '16px', background: 'rgba(255,255,255,0.95)', padding: '16px', borderRadius: '16px', boxShadow: '0 20px 40px rgba(0,0,0,0.5)', zIndex: 50 },
        title: { fontSize: '16px', fontWeight: '800', color: '#000', marginBottom: '2px' },
        date: { fontSize: '11px', color: '#666', marginBottom: '12px' },
        statRow: { display: 'flex', justifyContent: 'space-between' },
        statVal: { fontSize: '16px', fontWeight: '800', color: '#000' },
        statLbl: { fontSize: '9px', color: '#888', textTransform: 'uppercase', fontWeight: '600' }
      };
    }
    // 4. SUNSET — Hero big number (distance) style, warm bottom gradient
    if (shareTheme === 'SUNSET') {
      return {
        wrapper: { position: 'absolute', bottom: 0, left: 0, right: 0, padding: '32px 20px', background: 'linear-gradient(to top, rgba(120,30,0,0.95) 0%, rgba(80,20,0,0.7) 40%, transparent 100%)', zIndex: 50 },
        title: { fontSize: '12px', fontWeight: '700', color: '#ffb088', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '6px' },
        date: { display: 'none' },
        statRow: { display: 'flex', flexDirection: 'column', gap: '4px' },
        statVal: { fontSize: '32px', fontWeight: '900', color: '#fff', lineHeight: '1.1' },
        statLbl: { fontSize: '10px', color: '#ffb088', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '8px' }
      };
    }
    // 5. MIDNIGHT — Centered glass pill container
    if (shareTheme === 'MIDNIGHT') {
      return {
        wrapper: { position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', background: 'rgba(15,10,46,0.85)', backdropFilter: 'blur(20px)', padding: '24px 32px', borderRadius: '24px', border: '1px solid rgba(129,140,248,0.2)', boxShadow: '0 30px 60px rgba(0,0,0,0.5), 0 0 80px rgba(129,140,248,0.1)', zIndex: 50, display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center' },
        title: { display: 'none' },
        date: { display: 'none' },
        statRow: { display: 'flex', flexDirection: 'column', gap: '16px', alignItems: 'center' },
        statVal: { fontSize: '24px', fontWeight: '900', color: '#c7d2fe', textShadow: '0 0 30px rgba(129,140,248,0.6)' },
        statLbl: { fontSize: '10px', color: '#818cf8', textTransform: 'uppercase', letterSpacing: '2px', marginTop: '2px' }
      };
    }
    // 6. CARBON — Industrial horizontal bar at bottom
    if (shareTheme === 'CARBON') {
      return {
        wrapper: { position: 'absolute', bottom: '24px', left: '16px', right: '16px', background: 'rgba(24,24,27,0.92)', backdropFilter: 'blur(16px)', padding: '14px 18px', borderRadius: '14px', border: '1px solid rgba(228,228,231,0.08)', zIndex: 50, boxShadow: '0 20px 40px rgba(0,0,0,0.6)' },
        title: { display: 'none' },
        date: { display: 'none' },
        statRow: { display: 'flex', justifyContent: 'space-around', alignItems: 'center' },
        statVal: { fontSize: '18px', fontWeight: '900', color: '#e4e4e7', letterSpacing: '-0.5px', fontFamily: '"Outfit", monospace' },
        statLbl: { fontSize: '8px', color: '#52525b', textTransform: 'uppercase', letterSpacing: '2px', fontWeight: '700', marginTop: '2px' }
      };
    }
    // 7. RETRO — Boxed sections with decorative borders, left-aligned
    if (shareTheme === 'RETRO') {
      return {
        wrapper: { position: 'absolute', top: '8%', left: '20px', zIndex: 50, display: 'flex', flexDirection: 'column', gap: '0' },
        title: { display: 'none' },
        date: { display: 'none' },
        statRow: { display: 'flex', flexDirection: 'column', gap: '0' },
        statVal: { fontSize: '28px', fontWeight: '900', color: '#fbbf24', textShadow: '0 0 20px rgba(251,191,36,0.3)', borderLeft: '3px solid #fbbf24', paddingLeft: '12px', lineHeight: '1.2' },
        statLbl: { fontSize: '10px', color: '#a16207', textTransform: 'uppercase', letterSpacing: '2px', paddingLeft: '15px', marginBottom: '12px' }
      };
    }
    // 8. FROST — Right-aligned, stacked with thin line separators
    if (shareTheme === 'FROST') {
      return {
        wrapper: { position: 'absolute', top: '8%', right: '20px', zIndex: 50, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', textAlign: 'right' },
        title: { display: 'none' },
        date: { display: 'none' },
        statRow: { display: 'flex', flexDirection: 'column', gap: '0', alignItems: 'flex-end' },
        statVal: { fontSize: '26px', fontWeight: '900', color: '#e0f2fe', textShadow: '0 0 25px rgba(6,182,212,0.4)', lineHeight: '1.2' },
        statLbl: { fontSize: '9px', color: '#06b6d4', textTransform: 'uppercase', letterSpacing: '2px', marginBottom: '10px', borderBottom: '1px solid rgba(6,182,212,0.3)', paddingBottom: '8px' }
      };
    }
    ...
    if (shareTheme === 'LAVA') {
      return {
        ...
        statVal: { fontSize: '26px', fontWeight: '900', color: '#fca5a5', textShadow: '0 0 30px rgba(239,68,68,0.6)', paddingTop: '10px' },
        statLbl: { fontSize: '10px', color: '#ef4444', textTransform: 'uppercase', letterSpacing: '2px', marginTop: '2px', paddingBottom: '10px', borderBottom: '1px solid rgba(239,68,68,0.3)', width: '100px', textAlign: 'center' }
      };
    }
    // Default: STRAVA_DARK / Pro — Centered vertical, clean
    return {
      wrapper: { position: 'absolute', top: '32px', left: 0, right: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', color: '#fff', zIndex: 50 },
      title: { display: 'none' },
      date: { display: 'none' },
      statRow: { display: 'flex', flexDirection: 'column', gap: '14px', alignItems: 'center', textAlign: 'center' },
      statVal: { fontSize: '24px', fontWeight: '900', textShadow: '0 4px 20px rgba(0,0,0,0.8)', display: 'flex', alignItems: 'baseline', justifyContent: 'center', gap: '4px', lineHeight: '1' },
      statLbl: { fontSize: '10px', color: '#888', textTransform: 'uppercase', letterSpacing: '2px', marginTop: '2px', textShadow: '0 2px 10px rgba(0,0,0,0.8)' }
    };
  };
